...
const QUERY_QUOTA_URL = 'https://api.weixin.qq.com/wxa/queryquota';
const SPEED_UP_AUDIT_URL = 'https://api.weixin.qq.com/wxa/speedupaudit';
...
async function getQuota(accessToken) {
  let url = `${QUERY_QUOTA_URL}?access_token=${accessToken}`;
  let opts = {
    dataType: 'json',
    method: 'POST',
    headers: { 'Content-Type': 'application/json' }
...
async function speedUpAudit(accessToken, auditId) {
  let url = `${SPEED_UP_AUDIT_URL}?access_token=${accessToken}`;
  let opts = {
    dataType: 'json',
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
