...
// callback(err, media_id)
function uploadPermanentMaterial(access_token, filepath, mediaUrl, title, introduction, type, callback) {
  var options = ["thumb", "image", "video", "voice"];
  console.log(filepath, mediaUrl, title, introduction, type);
  ...
  if (options.indexOf(type) === -1)
    ...
    if (filepath) {
      ...
      let urlSplit = mediaUrl.split('/');
      let name = urlSplit[urlSplit.length - 1];
      console.log(name);
      console.log(mediaUrl);
      form.buffer('media', urllib.request(mediaUrl).pipe(fs.createWriteStream(name)));
    }
    ...
    urllib.request(url, opts, function (err, body, resp) {
      ...
      if (resp.statusCode !== 200) {
       return callback("statusCode not 200");
     }

     if (!body) {
       return callback("response body is null");
     }
